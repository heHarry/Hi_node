...
const register = (req, res) => {
    ...
    if (body.username.trim().length <= 0 || body.password.trim().length <= 0 || body.nickname.trim().length <= 0) {
        return res.send({ status: 501, msg: "重新输入信息" })
    }
    // 判断信息是否重复
...
const login = (req, res) => {
    const body = req.body
    console.log(body)
    if(!body.username) {return res.send({msg:"请正确输入用户名",status:404})}
    if(!body.password) {return res.send({msg:"请输入正确密码",status:404})}
    const sql = "select * from users where  username = ? and password = ?  "
    conn.query(sql, [body.username, body.password], (err, result) => {
        if (err) return res.status(501).send({ msg: "请重新输入" })
        if (result.length !== 1) return res.send({ msg: "请重新登录", status: 505 })
